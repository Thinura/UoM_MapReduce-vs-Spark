...
df = spark.read.format("csv").option("header", "true").load(
    "s3://airlinedelay/Input/DelayedFlights-updated.csv")
...
for iteration in range(number_iterations):
    iteration_key = f'Spark {iteration + 1}'
    print("=====================", iteration_key, "==================")
    for query_number, query in enumerate(queries, start=1):
        start_time = time.time()
        queryExecution = spark.sql(query)
        results = queryExecution.collect()
        end_time = time.time()
        print(results)
        ...
        print(f"Time Taken to execute query {query_number}: {end_time - start_time} seconds")
...
for i in range(len(data['Query'])):
    sum_value = sum(
        data[f'Spark {j}'][i] for j in range(1, num_iterations + 1) if data[f'Spark {j}'][i] is not None)
...
export_table_as_png(df, './spark/tables/time_data_query_results.png',title="Time Data Query", fontsize=6)

# Save DataFrame to CSV
df.to_csv('./spark/csv/time_data_query_results.csv', index=False)
...
for i, (query, data) in enumerate(query_results.items(), start=1):
    ...
    export_table_as_png(df, f'./spark/tables/Query_{i}_query_results.png', title=query,fontsize=6)


# Write data to a single CSV file
with open("./spark/csv/query_results.csv", 'w', newline='') as csvfile:
